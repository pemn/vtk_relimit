...
def vtk_mesh_to_df(mesh, face_size = None, xyz = ['x','y','z'], n0 = 0):
  ...
  if isinstance(mesh, list) or hasattr(mesh, 'n_blocks'):
    ...
    if mesh.GetDataObjectType() in [2,4,6]:
      ...
      if len(mesh.cell_arrays) == 0 and len(mesh.point_arrays) > 0:
        mesh = mesh.ptc()
...
def mesh_rotate_z_0261(mesh, angle, origin):
  if pv.__version__ == '0.26.1':
    mesh.translate(np.multiply(origin,-1))
    mesh.rotate_z(angle)
    mesh.translate(origin)
  else:
    mesh.rotate_z(angle, origin)
  return mesh
...
class vtk_Voxel_(object):
  ...
  def rotate_z_origin(self, bearing, point):
    ...
    mesh_rotate_z_0261(self, r, point)
  ...
  def from_bb_schema(cls, bb, schema, ndim = 3):
    ...
    if bearing != 0:
      ...
      mesh_rotate_z_0261(mesh, bearing * -1, bb[0])
      mesh = mesh.outline()
    ...
    if bearing != 0:
      ...
      self = vtk_VoxelSG(self.cast_to_structured_grid())
      mesh_rotate_z_0261(self, bearing, bb[0])
...
def vtk_grid_flag_ijk(grid, flag_var = None):
  if not flag_var:
    flag_var = ijk
